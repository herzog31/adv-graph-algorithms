 ...
 * Teilweise aus dem TUM Styleguide.
 * @type Object
 */
var const_Colors = {
...
var global_Edgelayout = {
    ...
    'labelColor' : "black"	// Farbe der Kantenbeschriftung
};
...
function GraphNode(coordinates, nodeID) {
     ...
     * @type Object
     */
    var layout = jQuery.extend({}, global_NodeLayout);
    // Kopie
    ...
    /**
     * Alphabetischer Name für Knoten
     * @type String
     */
    var name = null;
     ...
     * @method
     * @returns {String} Text des Knotens
     */
    ...
    /**
     * Setzt einen Text in den Knoten
     * @method
     * @param {String} newLabel Neuer Text für Knoten.
     */
    ...
    /**
     * Gibt dem Knoten einen Namen
     * @method
     * @param {String} newName Neuer Name für Knoten.
     */
...
GraphNode.prototype.draw = function(ctx) {
    CanvasDrawMethods.drawEllipse(ctx, this.getCoordinates(), this.getLayout(), this.getLabel().toString());
};

/**
 * Zeigt, ob sich die gegebenen Koordinaten auf dem Knoten befinden.
 * @method
...
GraphNode.prototype.contains = function(mx, my) {
    ...
    // fit radius to ellipse
    var rx = radius + 10;
    var ry = rx * 0.75;
    return (Math.pow((mx - coord.x) / rx, 2) + Math.pow((my - coord.y) / ry, 2) <= 1);
};
...
Edge.prototype.contains = function(mx, my, ctx) {
    var toleranz = 7;
    // Wie viele Punkte entfernt von der Kante darf man klicken?
    ...
    var MouseShiftRot = {
        x : MouseShift.x * Math.cos(-alpha) - MouseShift.y * Math.sin(-alpha),
        y : MouseShift.x * Math.sin(-alpha) + MouseShift.y * Math.cos(-alpha)
    };
    var targetShift = {
        x : targetC.x - sourceC.x,
        y : targetC.y - sourceC.y
    };
    var targetShiftRot = {
        x : targetShift.x * Math.cos(-alpha) - targetShift.y * Math.sin(-alpha),
    ...
    var center = {
        x : (targetC.x + sourceC.x) / 2,
        y : (targetC.y + sourceC.y) / 2
    };
    ...
    var c0 = {
        x : center.x + Math.cos(alpha) * labelWidth / 2,
    ...
    var c11 = {
        x : c1.x + Math.cos(alpha + Math.PI / 2) * (-3 - arrowHeight - this.getLayout().fontSize),
    ...
    var upperCorner = {
        x : upperCornerOld.x * Math.cos(-alpha) - upperCornerOld.y * Math.sin(-alpha),
    ...
    var rotatedMouse = {
        x : rotatedMouseOld.x * Math.cos(-alpha) - rotatedMouseOld.y * Math.sin(-alpha),
...
Edge.prototype.shift = function() {
    ...
    var alpha = Math.atan2(targetC.y - sourceC.y, targetC.x - sourceC.x);
    // Winkel der aktuellen Kante
    this.setSourceCoordinates({
        x : sourceC.x - 6 * Math.cos(alpha + Math.PI / 2),
        y : sourceC.y - 6 * Math.sin(alpha + Math.PI / 2)
    }, true);
    this.setTargetCoordinates({
        x : targetC.x - 6 * Math.cos(alpha + Math.PI / 2),
        y : targetC.y - 6 * Math.sin(alpha + Math.PI / 2)
    }, true);
};
...
function Graph(filename, canvas) {
    ...
    this.addNode = function(xC, yC) {
        ...
        var text = this.mapNodeIDToName(nodeIDCounter);
        node.setLabel(text);
        node.setName(text);
        nodeIDCounter++;
        return node;
     ...
     * @return {String} name
     */
    this.mapNodeIDToName = function(nodeID) {
	...
	// Verringere nodeIDCounter auf maximale (im Graphen existente) NodeID + 1
	while (nodeIDCounter > 0 && typeof this.nodes[nodeIDCounter - 1] == "undefined") {
		nodeIDCounter--;
	}
    };
    ...
    this.addEdge = function(source, target, weight) {
        ...
        if (weight == null) {
            weight = 1 + Math.round(Math.random() * 99);
            // Zufälliges Gewicht zwischen 1 und 100
        }
    ...
    this.restoreLayout = function() {
        ...
        for (var nodeID in this.nodes) {
            ...
            this.nodes[nodeID].setLabel(null);
            this.nodes[nodeID].setName(this.mapNodeIDToName(parseInt(nodeID)));
        }
        return oldLayout;
    };
        ...
        //     console.log(text);
            ...
            for (var line in lines) {
                ...
                if (parameter[0] == "e") {
                    ...
                    } else if (!isNaN(parseInt(parameter[1])) && !isNaN(parseInt(parameter[2])) && parameter[3] == "?") {
                        var sourceId = parseInt(parameter[1]);
                        var targetId = parseInt(parameter[2]);
                        var weight = "?";
                        closure_graph.addEdge(closure_graph.nodes[sourceId], closure_graph.nodes[targetId], weight);
    ...
    function generateRandomGraph(canvas) {
        ...
        for (var i = 0; i < NumberOfNodes; i++) {
            ...
            // Knoten nicht zu nah am Rand
            var y = Math.random() * (canvas.height() - 100) + 50;
            x = Math.round(x / 10) * 10;
            // Knoten ein bisschen gleichmäßiger verteilt.
...
Graph.prototype.getEdgeBetween = function(source, target) {
    for (var edgeID in this.edges) {
        if (this.edges[edgeID].getSourceID() == source.getNodeID() && this.edges[edgeID].getTargetID() == target.getNodeID()) {
            return edgeID;
        }
    }
    return null;
...
function GraphDrawer(p_graph, p_canvas, p_tab) {
    ...
    this.registerEventHandlers = function() {
        canvas.on("mouseup.GraphDrawer", function() {
            algo.mouseUpHandler();
        });
        // Mouseup
        canvas.on("mousemove.GraphDrawer", function(e) {
            algo.mouseMoveHandler(e);
        });
        // Mousemove
        canvas.on("dblclick.GraphDrawer", function(e) {
            algo.dblClickHandler(e);
        });
        // Doppelklick
        canvas.on("contextmenu.GraphDrawer", function(e) {
            algo.rightClickHandler(e);
        });
        // Rechtklick
        canvas.on("mousedown.GraphDrawer", function(e) {
            algo.mouseDownHandler(e);
        });
        // Linksklick
        $("#tg_select_GraphSelector").on("change.GraphDrawer", function() {
            algo.setGraphHandler();
        });
        // Beispielgraph auswählen
    };
        ...
        if (selectedNode == null) {
            for (var kantenID in graph.edges) {
                if (graph.edges[kantenID].contains(e.pageX - canvas.offset().left, e.pageY - canvas.offset().top, this.canvas[0].getContext("2d"))) {
                    ...
                    this.showWeightChangeField(e, kantenID);
                    // Zeige Feld zum Verändern des Gewichts
                    this.needRedraw = true;
                    return;
                }
            }
    ...
    this.rightClickHandler = function(e) {
        ...
        // Kein Kontextmenü
        this.deselectNode();
        // In jedem Fall erstmal den aktuellen Knoten abwählen...
        ...
        for (var knotenID in graph.nodes) {
            if (graph.nodes[knotenID].contains(mx, my)) {
                ...
                this.needRedraw = true;
                return;
                // Immer nur einen Knoten löschen
            }
        }
        for (var kantenID in graph.edges) {
    ...
    this.showWeightChangeField = function(event, kantenID) {
        ...
        $("#WeightChangePopup_" + kantenID.toString()).append("<input id=\"WeightInput_" + kantenID.toString() + "\" value=" + algo.graph.edges[kantenID].weight.toString() + "></input>");
        $("#WeightChangePopup_" + kantenID.toString()).append("<button id=\"WeightSave_" + kantenID.toString() + "\">speichern</button>");
        $("#WeightInput_" + kantenID.toString()).spinner({
            culture : "de",
            min : 1
        });
        $("#WeightInput_" + kantenID.toString()).keydown(function(event) {
            if (event.which == $.ui.keyCode.ENTER) {
                var newWeightString = $("#WeightInput_" +kantenID.toString())[0].value.replace(",", ".");
        ...
        $("#WeightInput_" + kantenID.toString()).on("focusout", function() {
            $("#WeightSave_" + kantenID.toString()).trigger('click');
        });
    };
    ...
    this.setGraphHandler = function() {
        ...
        // Positionen in Legende setzen, wegen Bildnachweis erneut benötigt
        var legende = this.tab.find(".Legende");
        var parentPosition = this.canvas.offset();
        legende.offset({
            top : (parentPosition.top + this.canvas.height() - legende.height() - 1),
            left : (parentPosition.left + this.canvas.width() - legende.width() - 1)
        });
