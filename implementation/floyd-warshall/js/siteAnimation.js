...
function initializeSiteLayout() {
    ...
    $("#ti_button_gotoFA1").click(function() {
        $("#tabs").tabs("option", "active", 4);
    });
    $("#ti_button_gotoFA2").click(function() {
        $("#tabs").tabs("option", "active", 5);
    });
    $("#ti_button_gotoFA3").click(function() {
        $("#tabs").tabs("option", "active", 6);
    });
    $("#tw_Accordion").accordion({
        heightStyle : "content"
    });
    $("#tabs").tabs({
        beforeActivate : function(event, ui) {
            ...
            if (ui.oldPanel[0].id == "tab_ta") {
                if (!($("#tab_ta").data("algo").isFinished())) {
            ...
            if (ui.oldPanel[0].id == "tab_tf3") {
                if ($("#tabs").data("tabChangeDialogOpen") == null && $("#tab_tf3").data("algo").getWarnBeforeLeave()) {
                    event.preventDefault();
                    $("#tabs").data("requestedTab", $("#" + ui.newPanel.attr("id")).index() - 1);
                    $("#tabs").data("tabChangeDialogOpen", true);
                    $("#tf3_div_confirmTabChange").dialog("open");
                } else {
                    $("#tab_tf3").data("algo").destroy();
                }
            }
        },
...
function CanvasDrawer(p_graph, p_canvas, p_tab) {
    ...
    this.animateLegende = function() {
        legendeMaxButton.button({
            icons : {
                primary : "ui-icon-minus"
            },
            text : false
        });
        legendeMinButton.button({
            icons : {
        ...
        this.minimizeLegend();
        legendeMaxButton.on("click.CanvasDrawer", function() {
            algo.minimizeLegend();
        });
        legendeMinButton.on("click.CanvasDrawer", function() {
            algo.maximizeLegend();
        });
